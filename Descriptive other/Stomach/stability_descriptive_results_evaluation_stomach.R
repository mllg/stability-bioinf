...
pareto_inds <- paretos - 2 * nrow(results)
...
best <- lapply(1:length(measures_indices), function(i) 
  pfs(data = results_flip[results_pareto[[i]]$pareto_indices - 2 * nrow(results), ], 
      criteria_indices = c(which(colnames(results) == "mmce.test.mean"), measures_indices[i],
                          which(colnames(results) == "mean_size"))))
best_indices <- lapply(1:length(best), function(i) results_pareto[[i]]$pareto_indices[best[[i]]] - 2 * nrow(results))
